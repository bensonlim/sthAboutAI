...
**主要方法**：
- `materialBasicInfoBatchGet` - 批量获取物料基本信息（已过期）
...
- `materialLimitMaterialQuery` - 查询物料（已过期）
- `materialLimitMaterialBatchQuery` - 批量查询物料（已过期）
...
// 单个物料ID查询
List<Long> materialIds = Collections.singletonList(1001L);
List<MtMaterialSiteBaseVO> materialSites = mtMaterialRpcClient.queryMaterialSiteByMaterialId(tenantId, materialIds);

// 批量物料ID查询
List<Long> batchMaterialIds = Arrays.asList(1001L, 1002L, 1003L);
List<MtMaterialSiteBaseVO> batchMaterialSites = mtMaterialRpcClient.queryMaterialSiteByMaterialId(tenantId, batchMaterialIds);

// 遍历处理结果
for (MtMaterialSiteBaseVO site : materialSites) {
    System.out.println("物料站点ID: " + site.getMaterialSiteId());
    System.out.println("物料ID: " + site.getMaterialId());
    System.out.println("站点ID: " + site.getSiteId());
    System.out.println("站点编码: " + site.getSiteCode());
}
```

**说明**：
- `queryMaterialSiteByMaterialId` 方法返回的是 `List<org.tarzan.method.domain.vo.MtMaterialSiteBaseVO>` 类型
- 该方法用于根据物料ID查询物料站点信息，支持单个和批量查询
- 返回的对象包含 `materialSiteId`、`materialId`、`siteId`、`siteCode` 等字段
